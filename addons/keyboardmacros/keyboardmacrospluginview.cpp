...
void KeyboardMacrosPluginView::addNamedMacro(const QString &name, const QString &description)
{
    ...
    // add load action
    auto action = new QAction(i18nc("%1 is the name of a keyboard macro", "Load %1", label), this);
    action->setToolTip(i18nc("%1 is the name of a keyboard macro", "Load the ‘%1’ macro as the current one.", name));
    ...
    // add play action
    action = new QAction(i18nc("%1 is the name of a keyboard macro", "Play %1", label), this);
    action->setToolTip(i18nc("%1 is the name of a keyboard macro", "Play the ‘%1’ macro without loading it.", name));
    action->setEnabled(true);
    connect(action, &QAction::triggered, m_plugin, [this, name] {
        slotPlayNamed(name);
    });
    ...
    // add wipe action
    action = new QAction(i18nc("%1 is the name of a keyboard macro", "Wipe %1", label), this);
    action->setToolTip(i18nc("%1 is the name of a keyboard macro", "Wipe the ‘%1’ macro.", name));
    action->setEnabled(true);
    connect(action, &QAction::triggered, m_plugin, [this, name] {
        slotWipeNamed(name);
    });
...
void KeyboardMacrosPluginView::slotWipeNamed(const QString &name)
{
    ...
    if (QMessageBox::question(m_mainWindow->window(), i18n("Keyboard Macros"), i18nc("%1 is the name of a keyboard macro", "Wipe the ‘%1’ macro?", name)) != QMessageBox::Yes) {
        return;
    }
    m_plugin->wipe(name);
}
